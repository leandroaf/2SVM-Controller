...
public final class ContextChangeMsgTypeSupport implements TypeSupport {
	...
	public ContextChangeMsgTypeSupport() {
		ContextChangeMsg tmp = new ContextChangeMsg();
	...
	public DataReader create_datareader(Subscriber sub, TopicDescription td,
		...
		return new ContextChangeMsgDataReader(sub, td, jni_dr);
	}

	public DataWriter create_datawriter(Publisher pub, Topic topic,
			SWIGTYPE_p__DataWriter jni_dw) {
		return new ContextChangeMsgDataWriter(pub, topic, jni_dw);
	}

	// marshal variants
	public int marshall(ByteBuffer out_stream, ContextChangeMsg src) {
		int size = 0;
		if (out_stream == null) {
	...
	public int marshal_size_core(ByteBuffer out_stream, int size, ContextChangeMsg src) {
		// src.eventName
	...
	public int marshal_core(ByteBuffer out_stream, int offset, ContextChangeMsg src) {
	...
	public int marshall_key(ByteBuffer out_stream, ContextChangeMsg src) {
		int size = 0;
		if (out_stream == null) {
			size += 4; // for CDR 'header'
		} else {
	...
	public int marshall_key_hash(ByteBuffer out_stream, ContextChangeMsg src) {
		int size = 0;
		if (out_stream == null) {
		} else {
			int offset = 0;
	...
	// unmarshal variants
	public int unmarshall(ContextChangeMsg t, ByteBuffer data, int s) {
		int offset = 0;
	...
	public int unmarshal_core(ContextChangeMsg t, ByteBuffer data, int offset, int s) {
		// t.eventName
		{
			while ((offset & 0x03) != 0) {
				offset++;
	...
	public int unmarshall_key(ContextChangeMsg t, ByteBuffer data, int s) {
		int offset = 0;
		data.get(); // skip the first byte
		byte encoding = data.get(); // data encoding CDR / ENDIAN
		data.getShort(); // unused flags (2 bytes)
	...
	public int unmarshall_key_hash(ContextChangeMsg t, ByteBuffer data, int s) {
		int offset = 0;
		data.order(ByteOrder.BIG_ENDIAN);
		return 0;
	}
	...
	// <type> operations
	public ContextChangeMsg alloc() {
		return new ContextChangeMsg();
	}

	public void clear(ContextChangeMsg instance) {
		instance.clear();
	}

	public void destroy(ContextChangeMsg instance) { /* noop */
	}

	public void copy(ContextChangeMsg to, ContextChangeMsg from) {
		to.copy(from);
	}
