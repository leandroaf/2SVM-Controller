...
public final class NewSmartSpaceMsgTypeSupport implements TypeSupport{
  ...
  public int     marshal_size_core ( ByteBuffer out_stream, int size, NewSmartSpaceMsg src ) {
    // src.identificador
    size = (size+3) & 0xfffffffc;// align 4
    size += 4; // length
    if (src.deviceName == null) size += 1;
    else {
      try {
  ...
  // <type> operations
  public NewSmartSpaceMsg         alloc ()              { return new NewSmartSpaceMsg(); }
  public void       clear (NewSmartSpaceMsg instance)   { instance.clear(); }
  public void       destroy (NewSmartSpaceMsg instance) { /* noop */ }
  public void       copy (NewSmartSpaceMsg to, NewSmartSpaceMsg from) { to.copy(from); }
