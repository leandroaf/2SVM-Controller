...
public class UbiAppSub {

	public void newUbiApp() {
		...
		class TestDataReaderListener implements DataReaderListener {
			...
			public void on_data_available(DataReader dr) {
				...
				UbiAppMsgDataReader string_dr = (UbiAppMsgDataReader) dr;
				UbiAppMsgSeq samples = new UbiAppMsgSeq();
							...
							if (si.value[i].valid_data) {
								...
								new Thread() {
									public void run() {
										...
										eventHandler.newUbiApp(contextChange);
		...
		UbiAppMsgDataReader dr = (UbiAppMsgDataReader) sub.create_datareader(top,
